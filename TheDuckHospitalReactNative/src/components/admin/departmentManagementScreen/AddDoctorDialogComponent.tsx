...
interface AddDoctorDialogComponentProps {
  departmentId: number;
...
const AddDoctorDialogComponent = (props: AddDoctorDialogComponentProps) => {
  const {
    departmentId,
    refreshList,
    modalVisible,
  ...
  const handleAddStaffToDepartment = async () => {
    ...
    setLoading(true);
    const response = await addDoctorToDepartment(departmentId, staff.staffId);
    setLoading(false);
    if (response.success) {
...
export default AddDoctorDialogComponent;
