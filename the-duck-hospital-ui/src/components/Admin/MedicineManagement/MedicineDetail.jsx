...
function MedicineDetail(props) {
  const { medicine, handleGetMedicine } = props;
  let status = medicine.deleted;
  ...
  useEffect(() => {
    ...
    setStatusMedicine(status);
  }, [status]);

  const handleStatusChange = (event) => {
    setEditStatus(event.target.value);
    if (statusMedicine !== event.target.value) {
  ...
  const handleUpdateButtonClick = async () => {
    let response;
    if (statusMedicine) {
      response = await restoreMedicine(staff.staffId);
      if (response.success) {
      ...
      <BoxStyle>
        <Grid container alignItems={"center"} paddingBottom={1}>
          ...
          >
            ...
            <DialogConfirm
              open={deleteDialog}
              title={statusStaff ? "Mở khóa thuốc" : "Khóa thuốc"}
              content={
                statusStaff
